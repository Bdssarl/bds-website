// <auto-generated />
using System;
using Bds_site_web.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace bds_site_web_version2_.Migrations
{
    [DbContext(typeof(SiteWebBdsDbContext))]
    [Migration("20231231152710_AddSomeAttributesOnUser")]
    partial class AddSomeAttributesOnUser
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("Bds_site_web.Models.Actualite", b =>
                {
                    b.Property<string>("IdActualite")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Actualite");

                    b.Property<string>("CheminMediaActualite")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_media_actualite");

                    b.Property<string>("DescriptionActualite")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("description_actualite");

                    b.Property<string>("LibelleActualite")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("libelle_actualite");

                    b.HasKey("IdActualite")
                        .HasName("PRIMARY");

                    b.ToTable("actualite", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.AnnonceRecrutement", b =>
                {
                    b.Property<string>("IdAnnonceRecrutement")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Annonce_Recrutement");

                    b.Property<DateTime>("DateLimiteAnnonceRecrutement")
                        .HasColumnType("date")
                        .HasColumnName("date_limite_annonce_recrutement");

                    b.Property<string>("DescriptionAnnonceRecrutement")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("description_annonce_recrutement");

                    b.Property<string>("LibelleAnnonceRecrutement")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("libelle_Annonce_recrutement");

                    b.HasKey("IdAnnonceRecrutement")
                        .HasName("PRIMARY");

                    b.ToTable("annonce_recrutement", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Consulter", b =>
                {
                    b.Property<string>("EbooksIdEbook")
                        .HasColumnType("varchar(25)");

                    b.Property<string>("UsersId")
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime>("DateConsulter")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("IdConsulter")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("IdEbook")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("IdUser")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("EbooksIdEbook", "UsersId");

                    b.HasIndex("UsersId");

                    b.ToTable("Consulters");
                });

            modelBuilder.Entity("Bds_site_web.Models.DemandeEmploi", b =>
                {
                    b.Property<string>("IdEmploiNavigationIdEmploi")
                        .HasColumnType("varchar(25)");

                    b.Property<string>("UsersId")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("CheminCvDemandeEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime>("DateEnvoiDemandeEmploi")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("DescriptionMessageEmploi")
                        .HasColumnType("longtext");

                    b.Property<string>("IdDemandeEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("IdEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("MessageDemandeEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("NomCvDemandeEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("ObjetMessageEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Periode")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("TypeEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("IdEmploiNavigationIdEmploi", "UsersId");

                    b.HasIndex("UsersId");

                    b.ToTable("DemandeEmplois");
                });

            modelBuilder.Entity("Bds_site_web.Models.DemandeStage", b =>
                {
                    b.Property<string>("IdStageNavigationIdStage")
                        .HasColumnType("varchar(25)");

                    b.Property<string>("UsersId")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("CheminCvDemandeStage")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("DateEnvoiDemandeStage")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("DescriptionMessage")
                        .HasColumnType("longtext");

                    b.Property<string>("MessageDemandeStage")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("NomCvDemandeStage")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("ObjetMessage")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Periode")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("TypeEmploi")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("IdStageNavigationIdStage", "UsersId");

                    b.HasIndex("UsersId");

                    b.ToTable("DemandeStages");
                });

            modelBuilder.Entity("Bds_site_web.Models.Ebook", b =>
                {
                    b.Property<string>("IdEbook")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Ebook");

                    b.Property<string>("CheminEbook")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_Ebook");

                    b.Property<DateTime>("DateCreationEbook")
                        .HasColumnType("date")
                        .HasColumnName("date_creation_Ebook");

                    b.Property<string>("ExtensionEbook")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("extension_Ebook");

                    b.Property<string>("NomEbook")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_Ebook");

                    b.Property<string>("TailleEbook")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("taille_Ebook");

                    b.HasKey("IdEbook")
                        .HasName("PRIMARY");

                    b.ToTable("ebook", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Emploi", b =>
                {
                    b.Property<string>("IdEmploi")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Emploi");

                    b.Property<string>("DomaineEmploi")
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("domaine_emploi");

                    b.Property<string>("TypeEmploi")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("type_emploi");

                    b.HasKey("IdEmploi")
                        .HasName("PRIMARY");

                    b.ToTable("emploi", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Formation", b =>
                {
                    b.Property<string>("IdFormation")
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("id_formation");

                    b.Property<DateTime>("DebutFormation")
                        .HasColumnType("date")
                        .HasColumnName("debut_formation");

                    b.Property<string>("DescriptionFormation")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("description_formation");

                    b.Property<DateTime>("FinFormation")
                        .HasColumnType("date")
                        .HasColumnName("fin_formation");

                    b.Property<string>("NomFormation")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("nom_formation");

                    b.Property<string>("StatutFormation")
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("statut_formation");

                    b.HasKey("IdFormation")
                        .HasName("PRIMARY");

                    b.ToTable("formation", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Image", b =>
                {
                    b.Property<string>("IdImage")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Image");

                    b.Property<string>("CheminImage")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_image");

                    b.Property<DateTime>("DateCreation")
                        .HasColumnType("date")
                        .HasColumnName("date_creation");

                    b.Property<string>("ExtensionImage")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("extension_image");

                    b.Property<string>("HauteurImage")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("hauteur_image");

                    b.Property<string>("LargeurImage")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("largeur_image");

                    b.Property<string>("NomImage")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_image");

                    b.Property<string>("TailleImage")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("taille_image");

                    b.Property<string>("TypeImage")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("type_image");

                    b.HasKey("IdImage")
                        .HasName("PRIMARY");

                    b.ToTable("image_", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Message", b =>
                {
                    b.Property<string>("IdMessage")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Message");

                    b.Property<DateTime>("DateMessage")
                        .HasColumnType("date")
                        .HasColumnName("date_message");

                    b.Property<string>("DescriptionMessage")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("description_message");

                    b.Property<string>("IdUser")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.Property<string>("ObjetMessage")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("objet_message");

                    b.HasKey("IdMessage")
                        .HasName("PRIMARY");

                    b.HasIndex("IdUser");

                    b.ToTable("message", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Metier", b =>
                {
                    b.Property<string>("IdMetier")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("Id_Metier");

                    b.Property<string>("CheminImageMetier")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("Chemin_Image_Metier");

                    b.Property<string>("DescriptionMetier")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("Decription_Metier");

                    b.Property<string>("NameImageMetier")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("Name_Image_Metier");

                    b.Property<string>("TitleMetier")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("Title_Metier");

                    b.HasKey("IdMetier")
                        .HasName("PRIMARY");

                    b.ToTable("metier", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Newsletter", b =>
                {
                    b.Property<string>("IdNewsletter")
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("id_newsletter");

                    b.Property<string>("CheminMediaNewsletter")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("chemin_media_newsletter");

                    b.Property<string>("DescriptionNewsletter")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("description_newsletter");

                    b.Property<string>("NomNewsletter")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("nom_newsletter");

                    b.HasKey("IdNewsletter")
                        .HasName("PRIMARY");

                    b.ToTable("newsletter", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Page", b =>
                {
                    b.Property<string>("IdPage")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Page");

                    b.Property<string>("IdActualite")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Actualite");

                    b.Property<DateTime>("DateTelechargement")
                        .HasColumnType("date")
                        .HasColumnName("date_telechargement");

                    b.Property<string>("LibellePage")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("libelle_page");

                    b.HasKey("IdPage", "IdActualite")
                        .HasName("PRIMARY");

                    b.HasIndex(new[] { "IdActualite" }, "fk_1_page");

                    b.ToTable("page_", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Partenaire", b =>
                {
                    b.Property<string>("IdPartenaire")
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("id_partenaire");

                    b.Property<string>("CheminImageTemoignage")
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("chemin_image__temoignage");

                    b.Property<string>("NomPartenaire")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("nom_partenaire");

                    b.HasKey("IdPartenaire")
                        .HasName("PRIMARY");

                    b.ToTable("partenaire", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.PostulerAnnonceRecrutement", b =>
                {
                    b.Property<string>("AnnonceRecrutementsIdAnnonceRecrutement")
                        .HasColumnType("varchar(25)");

                    b.Property<string>("UsersId")
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime>("DateEnvoiPostulerAnnonceRecrutement")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("IdAnnonceRecrutement")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("IdPostulerAnnonceRecrutement")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("MessagePostulerAnnonceRecrutement")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("AnnonceRecrutementsIdAnnonceRecrutement", "UsersId");

                    b.HasIndex("UsersId");

                    b.ToTable("PostulerAnnonceRecrutements");
                });

            modelBuilder.Entity("Bds_site_web.Models.ProjetAVenir", b =>
                {
                    b.Property<string>("IdProjetAVenir")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_projet_A_venir");

                    b.Property<string>("CheminImageProjetAVenir")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_image_projet_A_venir");

                    b.Property<DateTime>("DateDebutProjetAVenir")
                        .HasColumnType("date")
                        .HasColumnName("date_debut_projet_A_venir");

                    b.Property<DateTime>("DateFinProjetAVenir")
                        .HasColumnType("date")
                        .HasColumnName("date_fin_projet_A_venir");

                    b.Property<string>("DureeProjetAVenir")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("duree_projet_A_venir");

                    b.Property<string>("NomImageProjetAVenir")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_image_projet_A_venir");

                    b.Property<string>("NomProjetAVenir")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_projet_A_venir");

                    b.Property<string>("ObjectifProjetAVenir")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("objectif_projet_A_venir");

                    b.HasKey("IdProjetAVenir")
                        .HasName("PRIMARY");

                    b.ToTable("projet_a_venir", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.ProjetEnCour", b =>
                {
                    b.Property<string>("IdProjetEnCours")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_projet_En_cours");

                    b.Property<string>("CheminImageProjetEnCours")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_image_projet_En_cours");

                    b.Property<DateTime>("DateDebutProjetEnCours")
                        .HasColumnType("date")
                        .HasColumnName("date_debut_projet_En_cours");

                    b.Property<DateTime>("DateFinProjetEnCours")
                        .HasColumnType("date")
                        .HasColumnName("date_fin_projet_En_cours");

                    b.Property<string>("DureeProjetEnCours")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("duree_projet_En_cours");

                    b.Property<string>("NomImageProjetEnCours")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_image_projet_En_cours");

                    b.Property<string>("NomProjetEnCours")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_projet_En_cours");

                    b.Property<string>("ObjectifResultatRealise")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("objectif_resultat_realise");

                    b.Property<string>("ResumeDuProjetEnCours")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("resume_du_projet_en_cours");

                    b.HasKey("IdProjetEnCours")
                        .HasName("PRIMARY");

                    b.ToTable("projet_en_cours", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.ProjetRealise", b =>
                {
                    b.Property<string>("IdProjetRealise")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_projet_realise");

                    b.Property<string>("CheminImageProjetRealise")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_image_projet_realise");

                    b.Property<DateTime>("DateDebutProjetRealise")
                        .HasColumnType("date")
                        .HasColumnName("date_debut_projet_realise");

                    b.Property<DateTime>("DateFinProjetRealise")
                        .HasColumnType("date")
                        .HasColumnName("date_fin_projet_realise");

                    b.Property<string>("DureeProjetRealise")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("duree_projet_realise");

                    b.Property<string>("NomImageProjetRealise")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_image_projet_realise");

                    b.Property<string>("NomProjetRealise")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_projet_realise");

                    b.Property<string>("ObjectifResultatRealise")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("objectif_resultat_realise");

                    b.Property<string>("ResultatProjetRealise")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("resultat_projet_realise");

                    b.HasKey("IdProjetRealise")
                        .HasName("PRIMARY");

                    b.ToTable("projet_realises", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Recommandation", b =>
                {
                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)");

                    b.ToTable("recommandation", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Stage", b =>
                {
                    b.Property<string>("IdStage")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Stage");

                    b.Property<string>("DomaineStage")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("domaine_stage");

                    b.Property<string>("TypeStage")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("type_stage");

                    b.HasKey("IdStage")
                        .HasName("PRIMARY");

                    b.ToTable("stage", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Telecharger", b =>
                {
                    b.Property<string>("EbooksIdEbook")
                        .HasColumnType("varchar(25)");

                    b.Property<string>("UsersId")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Avis")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("DateTelechargement")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("IdEbook")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("IdTelechargement")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("idUser")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("EbooksIdEbook", "UsersId");

                    b.HasIndex("UsersId");

                    b.ToTable("Telechargers");
                });

            modelBuilder.Entity("Bds_site_web.Models.Temoignage", b =>
                {
                    b.Property<string>("IdTemoignage")
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("id_temoignage");

                    b.Property<string>("CheminImageTemoignage")
                        .HasMaxLength(40)
                        .HasColumnType("varchar(40)")
                        .HasColumnName("chemin_image_temoignage");

                    b.Property<string>("DescriptionTemoignage")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("description_temoignage");

                    b.Property<string>("NomTemoignage")
                        .IsRequired()
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("nom_temoignage");

                    b.Property<string>("RoleTemoignage")
                        .HasMaxLength(45)
                        .HasColumnType("varchar(45)")
                        .HasColumnName("role_temoignage");

                    b.HasKey("IdTemoignage")
                        .HasName("PRIMARY");

                    b.ToTable("temoignage", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.User", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("varchar(255)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("longtext");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("varchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("tinyint(1)");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("varchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("varchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("longtext");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("longtext");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("longtext");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("varchar(256)");

                    b.Property<string>("civilite")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.UserNewsletter", b =>
                {
                    b.Property<string>("EmailUser")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("Email_User");

                    b.HasKey("EmailUser")
                        .HasName("PRIMARY");

                    b.ToTable("UserNewsletter", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.UserToNewsLetter", b =>
                {
                    b.Property<string>("NewslettersIdNewsletter")
                        .HasColumnType("varchar(20)");

                    b.Property<string>("UserNewslettersEmailUser")
                        .HasColumnType("varchar(25)");

                    b.HasKey("NewslettersIdNewsletter", "UserNewslettersEmailUser");

                    b.HasIndex("UserNewslettersEmailUser");

                    b.ToTable("UserToNewsLetters");
                });

            modelBuilder.Entity("Bds_site_web.Models.Video", b =>
                {
                    b.Property<string>("IdVideo")
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("id_Video");

                    b.Property<string>("CheminVideo")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("chemin_video");

                    b.Property<DateTime>("DateCreationVideo")
                        .HasColumnType("date")
                        .HasColumnName("date_creation_video");

                    b.Property<string>("ExtensionVideo")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("extension_video");

                    b.Property<string>("NomVideo")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("nom_video");

                    b.Property<string>("TailleVideo")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("varchar(25)")
                        .HasColumnName("taille_video");

                    b.HasKey("IdVideo")
                        .HasName("PRIMARY");

                    b.ToTable("video", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("longtext");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("varchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("varchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("ClaimType")
                        .HasColumnType("longtext");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("longtext");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("ClaimType")
                        .HasColumnType("longtext");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("longtext");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("longtext");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("varchar(255)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("RoleId")
                        .HasColumnType("varchar(255)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Name")
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Value")
                        .HasColumnType("longtext");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("Bds_site_web.Models.Consulter", b =>
                {
                    b.HasOne("Bds_site_web.Models.Ebook", null)
                        .WithMany()
                        .HasForeignKey("EbooksIdEbook")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UsersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Bds_site_web.Models.DemandeEmploi", b =>
                {
                    b.HasOne("Bds_site_web.Models.Emploi", "IdEmploiNavigation")
                        .WithMany()
                        .HasForeignKey("IdEmploiNavigationIdEmploi")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UsersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("IdEmploiNavigation");
                });

            modelBuilder.Entity("Bds_site_web.Models.DemandeStage", b =>
                {
                    b.HasOne("Bds_site_web.Models.Stage", "IdStageNavigation")
                        .WithMany()
                        .HasForeignKey("IdStageNavigationIdStage")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UsersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("IdStageNavigation");
                });

            modelBuilder.Entity("Bds_site_web.Models.Message", b =>
                {
                    b.HasOne("Bds_site_web.Models.User", "user")
                        .WithMany("Messages")
                        .HasForeignKey("IdUser")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("user");
                });

            modelBuilder.Entity("Bds_site_web.Models.Page", b =>
                {
                    b.HasOne("Bds_site_web.Models.Actualite", "IdActualiteNavigation")
                        .WithMany("Pages")
                        .HasForeignKey("IdActualite")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_1_page");

                    b.Navigation("IdActualiteNavigation");
                });

            modelBuilder.Entity("Bds_site_web.Models.PostulerAnnonceRecrutement", b =>
                {
                    b.HasOne("Bds_site_web.Models.AnnonceRecrutement", null)
                        .WithMany()
                        .HasForeignKey("AnnonceRecrutementsIdAnnonceRecrutement")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UsersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Bds_site_web.Models.Telecharger", b =>
                {
                    b.HasOne("Bds_site_web.Models.Ebook", null)
                        .WithMany()
                        .HasForeignKey("EbooksIdEbook")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UsersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Bds_site_web.Models.UserToNewsLetter", b =>
                {
                    b.HasOne("Bds_site_web.Models.Newsletter", null)
                        .WithMany()
                        .HasForeignKey("NewslettersIdNewsletter")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.UserNewsletter", null)
                        .WithMany()
                        .HasForeignKey("UserNewslettersEmailUser")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("Bds_site_web.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Bds_site_web.Models.Actualite", b =>
                {
                    b.Navigation("Pages");
                });

            modelBuilder.Entity("Bds_site_web.Models.User", b =>
                {
                    b.Navigation("Messages");
                });
#pragma warning restore 612, 618
        }
    }
}
